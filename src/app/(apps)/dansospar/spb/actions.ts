...
export async function createSpbAction(formData: FormData) {
  try {
    ...
    if (!lingkunganId || !intensiId || !subjekId || !kategoriBantuan || !file || file.size === 0) {
      return { success: false, error: "Semua kolom dan lampiran file wajib diisi" };
    }
    ...
    const googleDriveFileId = await uploadFileToDrive(buffer, fileName, file.type);
    
    // Generate nomor SPB
    const countSpb = await prisma.spbRequest.count();
    const currentYear = new Date().getFullYear();
    const nomorSpb = `SPB/${currentYear}/${(countSpb + 1).toString().padStart(4, '0')}`;
