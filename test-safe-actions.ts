...
async function runTest() {
  ...
  const umkm = await prisma.umkmData.safeCreate({
    data: {
  ...
  const updatedUmkm = await prisma.umkmData.safeUpdate({
    where: { id: umkm.id },
  ...
  const deletedUmkm = await prisma.umkmData.softDelete({ id: umkm.id }, userId);
  ...
  const searchResult = await prisma.umkmData.findMany({
    where: { id: umkm.id }
  });
